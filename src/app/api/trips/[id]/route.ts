...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // ── Auto-fill addresses from coordinates when trip is completed
    // or when startPoint/endPoint are missing ──
    const newStatus = body.status
    const needStartPoint = !body.startPoint && (newStatus === 'completed' || (updateData.startPoint as any) === null)
    const needEndPoint = !body.endPoint && (newStatus === 'completed' || (updateData.endPoint as any) === null)

    if (needStartPoint || needEndPoint) {
      try {
        // Get the existing trip to find equipmentId and dates
        const existingTrip = await db.trip.findUnique({ where: { id } })
        if (existingTrip) {
          const tracker = await db.glonassTracker.findFirst({ where: { equipmentId: existingTrip.equipmentId } })
          const settings = await db.axentaSettings.findFirst()

          if (tracker && settings?.isActive && settings.apiUrl && settings.apiKey) {
            const startDt = existingTrip.startDate
            const endDt = existingTrip.endDate || (body.endDate ? new Date(body.endDate) : null)
            ...
            // Fetch start coordinates via snapshot API
            if (needStartPoint && startDt) {
              try {
                const snapRes = await fetch(`http://localhost:3000/api/glonass/snapshot?equipmentId=${existingTrip.equipmentId}&datetime=${encodeURIComponent(new Date(startDt).toISOString())}`, {
                  signal: AbortSignal.timeout(30000),
                })
                if (snapRes.ok) {
                  const snapData = await snapRes.json()
                  if (snapData.address) {
                    updateData.startPoint = snapData.address
                  } else if (snapData.lat && snapData.lng) {
                    // Try reverse geocoding
                    try {
                      const geoRes = await fetch(`http://localhost:3000/api/glonass/geocode?lat=${snapData.lat}&lng=${snapData.lng}`, {
                        signal: AbortSignal.timeout(10000),
                      })
                      if (geoRes.ok) {
                        const geoData = await geoRes.json()
                        if (geoData.address) updateData.startPoint = geoData.address
                      }
                    } catch { /* ignore */ }
                  }
                }
            ...
            // Fetch end coordinates via snapshot API
            if (needEndPoint && endDt) {
              try {
                const snapRes = await fetch(`http://localhost:3000/api/glonass/snapshot?equipmentId=${existingTrip.equipmentId}&datetime=${encodeURIComponent(new Date(endDt).toISOString())}`, {
                  signal: AbortSignal.timeout(30000),
                })
                if (snapRes.ok) {
                  const snapData = await snapRes.json()
                  if (snapData.address) {
                    updateData.endPoint = snapData.address
                  } else if (snapData.lat && snapData.lng) {
                    try {
                      const geoRes = await fetch(`http://localhost:3000/api/glonass/geocode?lat=${snapData.lat}&lng=${snapData.lng}`, {
                        signal: AbortSignal.timeout(10000),
                      ...
                      if (geoRes.ok) {
                        const geoData = await geoRes.json()
                        if (geoData.address) updateData.endPoint = geoData.address
                      }
                    } catch { /* ignore */ }
                  }
                }
        ...
        console.error('[Trip PUT] Auto-fill addresses error:', autoFillErr)
      }
    }
