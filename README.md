...
**Новые функции:**
- Загрузка данных датчиков в карточке рейса — кнопка «Загрузить с трекера» с автозаполнением пробега и топлива
- Статистика рейса из Axenta: пробег, скорости, расход топлива, заправки/сливы, моточасы, простои
- Компактная шапка — минимизирована до одной строки, статистика в виде цветных бейджей

**Улучшения:**
- Автообновление карты (autoRefreshMs + onRefresh)
- Оптимизация кода компонента карты

**Исправления:**
- Критическая ошибка: зависший next-server не убивался при рестарте (500 на CSS/JS)
- Статика теперь всегда перезаписывается при рестарте сервера
- Добавлены недостающие иконки

### v1.1.0 (2026-05-15)

- Начальная публикация на GitHub
- Полный функционал: учёт техники, сотрудники, ремонты, экипажи, рейсы
- Интеграция с Axenta.cloud ГЛОНАСС
- Карта Leaflet с треками и позициями техники
- XLSX-отчёты
