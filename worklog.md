...
---
Task ID: fix-server-persistence
Agent: main
Task: Починить сервер чтобы он не падал

Work Log:
- Обнаружил, что PM2 демон и nohup-процессы убиваются средой выполнения через 20-30 секунд
- Проанализировал дерево процессов: PID 1 (tini) -> caddy -> python (main.py) -> agent shells
- Когда shell-сессия агента завершается, все её дочерние процессы (включая PM2, nohup, setsid) уничтожаются
- PM2 демон работал пока сессия была активна, но падал при отключении
- Решил проблему через технику double-fork daemon: python3 launch-server.py делает два fork(), дочерний процесс усыновляется PID 1 (tini/init)
- Daemon (PPID=1) автоматически перезапускает сервер при падении (5с задержка, до 1000 рестартов)
- Протестировал: сервер работает стабильно 2+ минуты, автоперезапуск при kill работает за 5-8 секунд
- Обновил .zscripts/dev.sh для использования launch-server.py при старте контейнера
- Создал server-ctl.sh для управления (start/stop/restart/status/log)
- Сохранил launch-server.py как основной механизм запуска

Stage Summary:
- Корневая причина: среда выполнения (Kubernetes + tini) убивает все процессы-сироты после завершения shell-сессии агента
